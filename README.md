<h1 align=center> BOOK👍</h1>


<div align="center">
    <img src=""   />
</div>



> [플레이 데이터] 한화시스템 BEYOND SW캠프 / 팀명 : hjhg..


🎬[Demo 시연영상](https://www.youtube.com/watch?v=dhMrKTwNI8U&lc=UgzCJR3WxkvsckRyyO94AaABAg&ab_channel=%EB%94%B0%EB%9D%BC%ED%95%98%EB%A9%B4%EC%84%9C%EB%B0%B0%EC%9A%B0%EB%8A%94IT)   
📃[프로젝트 회고록](블로그주소) 

<br>

## ✨ 프로젝트 설명

저희 서비스는 사용자가 본 웹툰/웹소설에 대해 별점 및 리뷰를 남기고 별점에 따라 사용자의 취향에 맞는 작품을 추천해주는 서비스입니다.
또한 매주 새롭게 올라오는 회차에 대한 라이브톡을 진행하여 독자 모두가 모여 이야기하며 작품에 대한 감상을 나눌 수 있습니다.  


## ✨ 프로젝트 배경

<div align="center">
    <img src="image1.png" width="60%"/>  
    <img src="image2.png" width="60%"/>
</div>

출처 : [닐슨코리아클릭](https://www.koreanclick.com/insights/newsletter_view.html?code=topic&id=586&page=1&utm_source=board&utm_medium=board&utm_campaign=topic&utm_content=20200826) |
[문화체육관광부 보도자료](https://www.mcst.go.kr/kor/s_notice/press/pressView.jsp?pSeq=20472)


현재 웹툰 및 웹소설을 드라마로 제작한 작품들이 성공을 거두며,
이로 인해 원작에 대한 관심도가 상승하고 있습니다. 
닐슨코리아클릭의 통계에 따르면, 2020년 7월에 웹툰과 웹소설의
총 이용시간은 전년 동월 대비 24% 증가했습니다. 
또한 문체부의 보고에 따르면, 웹소설 산업의 규모는 1조 390억 원으로 나타났습니다.
하지만 규모에 비해 작품 정보를 얻을 수 있는 충분한 곳이 부족한 상황입니다.

우리는 각 사용자의 취향과 선호도를 고려하여 맞춤형 추천 서비스를 제공하여 사용자들이 
작품 선택에 소비하는 시간을 최소화하고 더욱 즐거운 경험을 할 기회를 제공하고자 합니다.  

뿐만 아니라, 동일한 작품을 공유하는 사용자 간에는 서로 다른 시각과 해석을 나누며
다양한 관점을 탐험하여 더욱 만족스러운 작품 감상을 즐길 수 있습니다.


## 📌 프로젝트 시나리오

1. 사용자가 회원가입을 한다.   
    - 이메일를 입력합니다.
    - 패스워드를 입력합니다.
    - 발송된 인증 메일을 확인합니다.
    - 회원가입을 완료합니다.  
    

2. 사용자가 인증받은 이메일과 패스워드를 입력하여 로그인합니다.


3. 사용자는 마이페이지에 접속합니다.
    - 마이페이지에서는 닉네임을 설정합니다.
    - 마이페이지에서는 내가 평가한 작품, 나를 팔로우하는 회원 목록, 내가 팔로잉한 회원 목록, 취향 분석(선호 장르, 선호 작가 등)을 확인 할 수 있습니다.  


4. 작품을 선택하여 들어갑니다.
    - 작품에 대한 회차별 또는 전체 작품에 대한 코멘트를 남길 수 있습니다.
      - 코멘트 글자 수는 200자를 넘길 수 없습니다.
      - 욕설/비방어는 사용할 수 없습니다. 
    - 작품에 대한 회차별 또는 전체 작품에 대한 별점을 남길 수 있습니다.
    - 15개 이상의 작품을 평가할 경우 회차별 또는 전체 작품에 대한 내 예상 별점을 확인 할 수 있습니다.
    - 회차별 또는 전체 작품의 전체 평균 별점을 확인할 수 있습니다.
    - 작품의 평가 상태를 확인할 수 있습니다.
    - 작품명, 작가를 검색하여 작품을 찾을 수 있습니다.
    - 웹툰일 경우 (존재한다면) 원작 웹소설이나 제작된 드라마 등을 표시해줍니다.  


5. 사용자는 자신이 보는 웹툰/웹소설이 업로드 되는 날에 라이브톡에 들어갑니다.
    - 라이브톡은 상위 n개의 작품들에서만 진행됩니다.
    - 라이브톡에는 회원들만 참여가 가능합니다.
    - 12시가 되면 이전의 채팅들이 사라집니다
    - 채팅들이 사라지기 전에 사용자들은 채팅 내용을 백업할 수 있습니다.
    - 라이브톡에서 욕설/비방어를 입력할 경우 채팅을 보낼 수 없다는 메세지가 뜹니다.
    - 라이브톡에 비매너 회원이 있을 경우 신고를 할 수 있습니다.
    - 라이브톡은 상시로 참여할 수 있습니다.
    - 짧은 시간 내에 많은 개수의 채팅을 보낼 경우 도배 방지를 위해 일정 시간 동안 채팅이 금지됩니다.  


6. 사용자는 자신이 좋아하는 작품을 클릭해 해당 작품이 담겨있는 컬렉션을 봅니다.
    - 사용자는 자신이 좋아하는 영화, 자신이 보고 싶은 영화 등의 작품들을 담아서 컬렉션들을 만들 수 있습니다.
    - 회원 닉네임, 컬렉션 이름을 검색하여 다른 사용자가 만든 컬렉션을 볼 수 있습니다.
    - 작품 상세보기창에서 해당 작품이 담겨있는 다른 사용자들의 컬렉션을 볼 수 있습니다.  


7. 관리자가 작품을 등록합니다.
    - 네이버시리즈, 카카오페이지 등에서 작품 목록을 받아옵니다.
      - 등록 요청이 들어온 작품을 확인합니다.
      - 작품명, 작가, 작품구분(웹툰, 웹소설), 작품의 플랫폼, 회차 등을 입력합니다.


8. 관리자가 사용자 계정을 정지시킬 수 있습니다.
    - 사용자가 신고한 계정을 확인합니다.
    - 특정 횟수 이상으로 신고 당한 사람에 대해서는 블랙리스트에 추가합니다.

## 📌 업무 배경도




## 🔍 Overview

### 1. 사용자 회원 가입 및 로그인 기능(토큰 인증 방식)

<div align="center">
    <img src="."  />
나중에 사진 추가 예정
</div>

- 로그인
- 회원가입
- 마이페이지(팔로우 등)
- 관리자에게 작품 추가 요청하기

<br>

### 2. 작품

<div align="center">
    <img src=""  />
</div>

- 회차 별점 남기기
- 회차 코멘트 남기기
- 전체 별점 남기기
- 전체 코멘트 남기기
- 코멘트 글자 수 제한
- 금지 단어 필터링
- 코멘트 신고 기능
- 내 취향에 맞는 예상 별점 표시
- 전체 평균 별점 표시
- 평가 상태에 대한 표시(평가 완료 등..)
- 선호하는 플랫폼 선택 후 리스트 우선 순위로 출력

<br>


### 3. 라이브톡

<div align="center">
    <img src=""  />
</div>

- 채팅 (DB에는 다음 회차가 오픈되기 전 까지만 저장되며 다음 라이브톡이 열리면 삭제 후 갱신)
- 본인이 채팅 내용 저장 가능
- 라이브톡 신고 기능
- 도배 방지를 위하여 과도한 채팅을 여러번 보내면 채팅 입력 시간에 텀 주기
- 회원인 사람만 채팅에 참여 가능
- 욕설/비방어 필터링 처리(***이런 식으로)
- 상위 몇개 작품만 라이브톡 오픈
- 카톡 오픈 채팅방 처럼 상시로 오픈

<br>

### 4. 선호 작품 컬렉션

<div align="center">
    <img src=""  />
</div>

- 내가 봤던 혹은 좋았던 작품 목록 구성(노래 플레이리스트처럼)
- 프리미엄 유저(검증된 유저) 리스트 제작 (평론가처럼..)
- 컬렉션 비공개/공개 여부 설정

<br>

### 5. 검색

<div align="center">
    <img src=""  />
</div>

- 작품 검색
- 사람 검색
- 리스트 검색
- 연령대에 맞지 않은 작품은 필터링

<br>

### 6. 관리자

<div align="center">
    <img src=""  />
</div>

- 작품 등록
- 활동 중 신고 당한 사람에 대한 제재(블랙리스트 처리 등)

<br>

## 🖥 System Architecture



## 🤼‍♂️팀원

Team : 🐯 **김현균**

Team : 🐶 **이석하**

Team : 🐺 **이주현**

Team : 🐱 **전민재**

Team : 🦁 **홍현주**
